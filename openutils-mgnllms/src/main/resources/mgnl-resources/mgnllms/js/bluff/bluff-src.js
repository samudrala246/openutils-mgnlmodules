 ...
 * 
 * Documentation available at http://bluff.jcoglan.com Based on Gruff by
 * Geoffrey Grosenbach: http://github.com/topfunky/gruff
 * 
...
Bluff.Base = new JS.Class(
		{
			extend : {
				// Draw extra lines showing where the margins and text centers
				// are
				DEBUG : false,
			...
			// Get or set the list of colors that will be used to draw the bars
			// or lines.
			colors : null,
			...
			// Message shown when there is no data. Fits up to 20 characters.
			// Defaults
			// to "No Data."
			...
			// You can manually set a maximum value, such as a percentage-based
			// graph
			// that always goes to 100.
			//
			// If you use this, you must set it after you have given all your
			// data to
			// the graph object.
			maximum_value : null,

			// Set to false if you don't want the data to be sorted with largest
			// avg
			// values at the back.
			sort : null,
			...
			// Optionally set the size of the colored box by each item in the
			// legend.
			// Default is 20.0
			//
			// Will be scaled down if graph is smaller than 800px wide.
			...
			// If one numerical argument is given, the graph is drawn at 4/3
			// ratio
			// according to the given width (800 results in 800x600, 400 gives
			// 400x300,
			// etc.).
			//
			// Or, send a geometry string for other ratios ('800x400',
			// '400x225').
			...
			//
			// Subclasses can override this, call super, then set values
			// separately.
			//
			// This makes it possible to set defaults in a subclass but still
			// allow
			...
			//
			// If you specify fewer colors than the number of datasets you
			// intend
			// to draw, 'increment_color' will cycle through the array, reusing
			// colors as needed.
			//
			// Note that (as with the 'set_theme' method), you should set up the
			// color
			// list before you send your data (via the 'data' method). Calls to
			// the
			// 'data' method made prior to this call will use whatever color
			// scheme
			...
			// You can set a theme manually. Assign a hash to this method before
			// you
			// send your data.
			//
			// graph.set_theme({
			...
			set_theme : function(options) {
				...
				this.font_color = this._theme_options.font_color
						|| this.marker_color;
			...
			theme_keynote : function() {
				...
				this._orange = '#EFAA43';
				this._white = 'white';
				this.colors = [ this._yellow, this._blue, this._green,
						this._red, this._purple, this._orange, this._white ];
			...
			// A color scheme plucked from the colors on the popular usability
			// blog.
			theme_37signals : function() {
				...
				this._black = 'black';
				this.colors = [ this._yellow, this._blue, this._green,
						this._red, this._purple, this._orange, this._black ];
			...
			theme_rails_keynote : function() {
				...
				this._black = 'black';
				this.colors = [ this._green, this._grey, this._orange,
						this._red, this._white, this._light_grey, this._black ];
			...
			theme_odeo : function() {
				...
				this._black = 'black';
				this.colors = [ this._grey, this._white, this._dark_blue,
						this._dark_pink, this._green, this._light_grey,
						this._black ];
			...
			// Parameters are an array where the first element is the name of
			// the dataset
			...
			// graph.
			//
			// If the color argument is nil, the next color from the default
			// theme will
			// be used.
			//
			...
			data : function(name, data_points, color) {
				...
				data_points = Bluff.array(data_points); // make sure it's an
														// array
				this._data.push( [ name, data_points,
						(color || this._increment_color()) ]);
				// Set column count if this is larger than previous counts
				this._column_count = (data_points.length > this._column_count) ? data_points.length
						: this._column_count;
				...
				Bluff
						.each(
								data_points,
								function(data_point, index) {
									if (data_point === undefined)
										return;

									// Setup max/min so spread starts at the low
									...
									// Original: @maximum_value =
									// _larger_than_max?(data_point, index) ?
									// max(data_point, index) : @maximum_value
									this.maximum_value = this
											._larger_than_max(data_point) ? data_point
											: this.maximum_value;
									if (this.maximum_value >= 0)
										this._has_data = true;

									this.minimum_value = this
											._less_than_min(data_point) ? data_point
											: this.minimum_value;
			...
			draw : function() {
				...
				this._debug(function() {
					...
					this._d.rectangle(this.left_margin, this.top_margin,
							this._raw_columns - this.right_margin,
							this._raw_rows - this.bottom_margin);
					// Graph area box
					this._d.rectangle(this._graph_left, this._graph_top,
							this._graph_right, this._graph_bottom);
				});
			},
			...
			_setup_drawing : function() {
				// Maybe should be done in one of the following functions for
				// more granularity.
				if (!this._has_data)
					...
					Bluff
							.each(
									...
									function(data_row) {
										...
										Bluff
												.each(
														data_row[this.klass.DATA_VALUES_INDEX],
														function(data_point) {
															if (data_point === null
																...
																norm_data_points
																		.push((data_point - this.minimum_value)
																				/ this._spread);
														}, this);
										this._norm_data
												.push( [
			...
			_setup_graph_measurements : function() {
				this._marker_caps_height = this.hide_line_markers ? 0 : this
						._calculate_caps_height(this.marker_font_size);
				this._title_caps_height = this.hide_title ? 0 : this
						._calculate_caps_height(this.title_font_size);
				this._legend_caps_height = this.hide_legend ? 0 : this
						._calculate_caps_height(this.legend_font_size);

				var longest_label, longest_left_label_width, line_number_width, last_label, extra_room_for_long_label, x_axis_label_height, key;
				...
				if (this.hide_line_markers) {
					...
					if (this.has_left_labels) {
						...
						for (key in this.labels) {
							longest_label = longest_label.length > this.labels[key].length ? longest_label
									: this.labels[key];
						}
						longest_left_label_width = this._calculate_width(
								this.marker_font_size, longest_label) * 1.25;
					} else {
						longest_left_label_width = this._calculate_width(
								this.marker_font_size, this
										._label(this.maximum_value));
					}

					// Shift graph if left line numbers are hidden
					line_number_width = this.hide_line_numbers
							&& !this.has_left_labels ? 0.0
							: longest_left_label_width
									+ this.klass.LABEL_MARGIN * 2;

					this._graph_left = this.left_margin
							+ line_number_width
							+ (this.y_axis_label === null ? 0.0
									: this._marker_caps_height
											+ this.klass.LABEL_MARGIN * 2);

					// Make space for half the width of the rightmost column
					// label.
					// Might be greater than the number of columns if
					// between-style bar markers are used.
					last_label = -Infinity;
					for (key in this.labels)
						last_label = last_label > Number(key) ? last_label
								: Number(key);
					last_label = Math.round(last_label);
					extra_room_for_long_label = (last_label >= (this._column_count - 1) && this.center_labels_over_point) ? this
							._calculate_width(this.marker_font_size,
									this.labels[last_label]) / 2
							: 0;
					this._graph_right_margin = this.right_margin
				...
				this._graph_right = this._raw_columns
						- this._graph_right_margin;
				this._graph_width = this._raw_columns - this._graph_left
						- this._graph_right_margin;

				// When hide_title, leave a title_margin space for aesthetics.
				// Same with hide_legend
				this._graph_top = this.top_margin
						+ (this.hide_title ? this.title_margin
								: this._title_caps_height + this.title_margin)
						+ (this.hide_legend ? this.legend_margin
								: this._legend_caps_height + this.legend_margin);

				x_axis_label_height = (this.x_axis_label === null) ? 0.0
						: this._marker_caps_height + this.klass.LABEL_MARGIN;
				this._graph_bottom = this._raw_rows - this._graph_bottom_margin
						- x_axis_label_height;
			...
			_draw_axis_labels : function() {
				if (this.x_axis_label) {
					...
					var x_axis_label_y_coordinate = this._graph_bottom
							+ this.klass.LABEL_MARGIN * 2
							+ this._marker_caps_height;
					...
					this._d.pointsize = this
							._scale_fontsize(this.marker_font_size);
					this._d.gravity = 'north';
					this._d.annotate_scaled(this._raw_columns, 1.0, 0.0,
							x_axis_label_y_coordinate, this.x_axis_label,
							this._scale);
					this._debug(function() {
						this._d.line(0.0, x_axis_label_y_coordinate,
								this._raw_columns, x_axis_label_y_coordinate);
					});
				}
			...
			_draw_line_markers : function() {
				...
				if (this.y_axis_increment === null) {
					// Try to use a number of horizontal lines that will come
					// out even.
					//
					...
					if (this.marker_count === null) {
						Bluff.each( [ 3, 4, 5, 6, 7 ], function(lines) {
							if (!this.marker_count
									&& this._spread % lines === 0)
								this.marker_count = lines;
						}, this);
						this.marker_count = this.marker_count || 4;
					}
					this._increment = (this._spread > 0) ? this
							._significant(this._spread / this.marker_count) : 1;
				} else {
					...
					this.marker_count = Math.round(this._spread
							/ this.y_axis_increment);
					this._increment = this.y_axis_increment;
				}
				this._increment_scaled = this._graph_height
						/ (this._spread / this._increment);

				// Draw horizontal line markers and annotate with numbers
				var index, n, y, marker_label;
				for (index = 0, n = this.marker_count; index <= n; index++) {
					y = this._graph_top + this._graph_height - index
							* this._increment_scaled;
					...
					if (!this.hide_line_numbers) {
						...
						this._d.annotate_scaled(this._graph_left
								- this.klass.LABEL_MARGIN, 1.0, 0.0, y, this
								._label(marker_label), this._scale);
					}
				}
			},
			...
			// Draws a legend with the names of the datasets matched to the
			// colors used
			// to draw them.
			_draw_legend : function() {
				...
				var legend_square_width = this.legend_box_size; // small square
																// with color of
																// this item
				...
				Bluff
						.each(
								...
								function(label) {
									...
									var metrics = this._d
											.get_type_metrics(label);
									var label_width = metrics.width
											+ legend_square_width * 2.7;
									label_widths[last].push(label_width);

									if (Bluff.sum(label_widths[last]) > (this._raw_columns * 0.9))
										label_widths.push( [ label_widths[last]
												.pop() ]);
								}, this);

				var current_x_offset = this._center(Bluff.sum(label_widths[0]));
				var current_y_offset = this.hide_title ? this.top_margin
						+ this.title_margin : this.top_margin
						+ this.title_margin + this._title_caps_height;

				this._debug(function() {
					this._d.stroke_width = 1;
					this._d.line(0, current_y_offset, this._raw_columns,
							current_y_offset);
				});
						...
						.each(
								...
								function(legend_label, index) {
									...
									if (this.font)
										this._d.font = this.font;
									this._d.pointsize = this
											._scale_fontsize(this.legend_font_size);
									this._d.stroke = 'transparent';
									this._d.font_weight = 'normal';
									...
									this._d.pointsize = this.legend_font_size;
									var metrics = this._d
											.get_type_metrics(legend_label);
									var current_string_offset = metrics.width
											+ (legend_square_width * 2.7), line_height;

									// Handle wrapping
									label_widths[0].shift();
									if (label_widths[0].length == 0) {
										this._debug(function() {
											this._d.line(0.0, current_y_offset,
													this._raw_columns,
													current_y_offset);
										});
										...
										if (label_widths.length > 0)
											current_x_offset = this
													._center(Bluff
															.sum(label_widths[0]));
										line_height = Math.max(
												this._legend_caps_height,
												legend_square_width)
												+ this.legend_margin;
										if (label_widths.length > 0) {
											// Wrap to next line and shrink
											// available graph dimensions
											current_y_offset += line_height;
											this._graph_top += line_height;
											this._graph_height = this._graph_bottom
													- this._graph_top;
										}
									} else {
			...
			_draw_title : function() {
				...
				this._d.gravity = 'north';
				this._d.annotate_scaled(this._raw_columns, 1.0, 0,
						this.top_margin, this.title, this._scale);
			},
			...
			_draw_label : function(x_offset, index) {
				...
				if (this.labels[index] && !this._labels_seen[index]) {
					...
					this._d.pointsize = this
							._scale_fontsize(this.marker_font_size);
					this._d.gravity = 'north';
					this._d.annotate_scaled(1.0, 1.0, x_offset, y_offset,
					...
					this._debug(function() {
						this._d.stroke_width = 1;
						this._d
								.line(0.0, y_offset, this._raw_columns,
										y_offset);
					});
				}
			},
			...
			_draw_tooltip : function(left, top, width, height, name, color,
					data) {
				if (!this.tooltips)
					return;
			...
			_draw_no_data : function() {
				...
				this._d.annotate_scaled(this._raw_columns, this._raw_rows / 2,
						0, 10, this.no_data_message, this._scale);
			},

			// Finds the best background to render based on the provided theme
			// options.
			_render_background : function() {
				...
				default:
					this
							._render_image_background(this._theme_options.background_image);
					break;
				}
			},
			...
			_render_solid_background : function(color) {
				this._d.render_solid_background(this._columns, this._rows,
						color);
			},

			// Use with a theme definition method to draw a gradiated
			// background.
			_render_gradiated_background : function(top_color, bottom_color) {
				this._d.render_gradiated_background(this._columns, this._rows,
			...
			_sums : function(data_set) {
				var total_sum = 0;
				Bluff.each(data_set, function(num) {
					total_sum += (num || 0)
				});
				return total_sum;
			},
			...
			_make_stacked : function() {
				...
				Bluff.each(this._data, function(value_set) {
					Bluff.each(value_set[this.klass.DATA_VALUES_INDEX],
							function(value, index) {
								stacked_values[index] += value;
							}, this);
					value_set[this.klass.DATA_VALUES_INDEX] = Bluff
							.array(stacked_values);
				}, this);
			},
			...
			_increment_color : function() {
				...
				this._color_index = (this._color_index + 1)
						% this.colors.length;
				return this.colors[offset];
			},

			// Return a formatted string representing a number value that should
			// be
			// printed as a label.
			_label : function(value) {
				var sep = this.klass.THOUSAND_SEPARATOR, label = (this._spread
						% this.marker_count == 0 || this.y_axis_increment !== null) ? String(Math
						.round(value))
						: String(Math.floor(value * this._significant_digits)
								/ this._significant_digits);
			...
			// Returns the height of the capital letter 'X' for the current font
			// and
			// size.
			//
...
Bluff.Area = new JS.Class(Bluff.Base, {
	...
	draw : function() {
		...
		Bluff.each(this._norm_data, function(data_row) {
			var poly_points = [], prev_x = 0.0, prev_y = 0.0;

			Bluff.each(data_row[this.klass.DATA_VALUES_INDEX],
					function(data_point, index) {
						// Use incremented x and scaled y
						var new_x = this._graph_left
								+ (this._x_increment * index);
						var new_y = this._graph_top
								+ (this._graph_height - data_point
										* this._graph_height);
						...
						if (prev_x > 0 && prev_y > 0) {
							...
							// this._d.polyline(this._graph_left,
							// this._graph_bottom, new_x, new_y);
						}
...
Bluff.BarConversion = new JS.Class( {
	...
	getLeftYRightYscaled : function(data_point, result) {
		...
		case 1: // Case one
			...
			result[0] = this.graph_top + this.graph_height * (1 - data_point)
					+ 1;
		...
		case 2: // Case two
			...
			result[1] = this.graph_top + this.graph_height * (1 - data_point)
					- 1;
			break;
		case 3: // Case three
			...
			if (data_point >= this.zero) {
				result[0] = this.graph_top + this.graph_height
						* (1 - (val - this.zero)) + 1;
				result[1] = this.graph_top + this.graph_height
						* (1 - this.zero) - 1;
			} else {
				result[0] = this.graph_top + this.graph_height
						* (1 - (val - this.zero)) + 1;
...
Bluff.Bar = new JS.Class(
		Bluff.Base,
		{
			...
			draw : function() {
				...
				// there are more labels than columns. This is basically the
				// same
			...
			_draw_bars : function() {
				this._bar_width = this._graph_width
						/ (this._column_count * this._data.length);
				...
				// Set up the right mode [1,2,3] see BarConversion for further
				// explanation
				if (this.minimum_value >= 0) {
								...
								function(data_row, row_index) {
									...
									Bluff
											.each(
													...
													function(data_point,
															point_index) {
														// Use incremented x and
														// scaled y
														// x
														var left_x = this._graph_left
																+ (this._bar_width * (row_index
																		+ point_index + ((this._data.length - 1) * point_index)))
																+ padding;
														var right_x = left_x
																+ this._bar_width
																* this.bar_spacing;
														...
														var label_center = this._graph_left
																+ (this._data.length
																		* this._bar_width * point_index)
																+ (this._data.length
																		* this._bar_width / 2.0);
														// Subtract half a bar
														// width to center left
														// if requested
														this
																._draw_label(
																		label_center
																				- (this.center_labels_over_point ? this._bar_width / 2.0
																						: 0.0),
																		point_index);
													}, this);
...
// g.draw();
//
// There are also other options described below, such as #baseline_value,
// #baseline_color, #hide_dots, and #hide_lines.
...
Bluff.Line = new JS.Class(
		Bluff.Base,
		{
			...
			// Dimensions of lines and dots; calculated based on dataset size if
			// left unspecified
			line_width : null,
			dot_radius : null,

			// Hide parts of the graph to fit more datapoints, or for a
			...
			//
			// g = new Bluff.Line('canvasId', 400, false) // 400px wide, no
			// lines (for backwards compatibility)
			//
			// g = new Bluff.Line('canvasId', false) // Defaults to 800px wide,
			// no lines (for backwards compatibility)
			// 
			...
			draw : function() {
				...
				// Check to see if more than one datapoint was given. NaN can
				// result otherwise.
				this.x_increment = (this._column_count > 1) ? (this._graph_width / (this._column_count - 1))
						: this._graph_width;

				var level;

				if (this._norm_baseline !== undefined) {
					level = this._graph_top
							+ (this._graph_height - this._norm_baseline
									* this._graph_height);
					this._d.push();
					...
					this._d.line(this._graph_left, level, this._graph_left
							+ this._graph_width, level);
					this._d.pop();
				}
				...
				Bluff
						.each(
								...
								function(data_row, row_index) {
									...
									this._one_point = this
											._contains_one_point_only(data_row);
									...
									Bluff
											.each(
													...
													function(data_point, index) {
														var new_x = this._graph_left
																+ (this.x_increment * index);
														if (typeof data_point !== 'number')
															return;

														this._draw_label(new_x,
																index);
														...
														this._d.stroke_width = this.line_width
																|| this
																		._clip_value_if_greater_than(
																				this._columns
																						/ (this._norm_data[0][this.klass.DATA_VALUES_INDEX].length * 6),
																				3.0);
														...
														var circle_radius = this.dot_radius
																|| this
																		._clip_value_if_greater_than(
																				this._columns
																						/ (this._norm_data[0][this.klass.DATA_VALUES_INDEX].length * 2),
																				7.0);
			...
			_normalize : function() {
				...
				if (this.baseline_value !== null)
					this._norm_baseline = this.baseline_value
							/ this.maximum_value;
			},

			_contains_one_point_only : function(data_row) {
				// Spin through data to determine if there is just one value
				// present.
				var count = 0;
				Bluff.each(data_row[this.klass.DATA_VALUES_INDEX], function(
						data_point) {
					if (data_point !== undefined)
						count += 1;
				});
...
Bluff.Dot = new JS.Class(
		Bluff.Base,
		{
			...
			draw : function() {
				...
				this._item_width = this._items_width * spacing_factor
						/ this._norm_data.length;
				this._d.stroke_opacity = 0.0;
				var height = Bluff.array_new(this._column_count, 0), length = Bluff
						.array_new(this._column_count, this._graph_left), padding = (this._items_width * (1 - spacing_factor)) / 2;
									...
									Bluff
											.each(
													...
													function(data_point,
														...
														var x_pos = this._graph_left
																+ (data_point * this._graph_width)
																- Math
																		.round(this._item_width / 6.0);
														var y_pos = this._graph_top
																+ (this._items_width * point_index)
																+ padding
																+ Math
																		.round(this._item_width / 2.0);
														...
														this._d
																.circle(
																		...
																		x_pos
																				+ Math
																						.round(this._item_width / 3.0),
																		y_pos);

														// Calculate center
														// based on item_width
														// and current row
														var label_center = this._graph_top
																+ (this._items_width
																		* point_index + this._items_width / 2)
																+ padding;
														this._draw_label(
																label_center,
																point_index);
			...
			// Instead of base class version, draws vertical background lines
			// and label
			_draw_line_markers : function() {
				...
				// TODO Round maximum marker value to a round number like 100,
				// 0.1, 0.5, etc.
				var increment = this._significant(this.maximum_value
						/ number_of_lines);
				for ( var index = 0; index <= number_of_lines; index++) {

					var line_diff = (this._graph_right - this._graph_left)
							/ number_of_lines, x = this._graph_right
							- (line_diff * index) - 1, diff = index
							- number_of_lines, marker_label = Math.abs(diff)
							* increment;

					this._d.stroke = this.marker_color;
					this._d.line(x, this._graph_bottom, x, this._graph_bottom
							+ 0.5 * this.klass.LABEL_MARGIN);
					...
					if (!this.hide_line_numbers) {
						...
						this._d.annotate_scaled(0, 0, // Width of box to draw
														// text in
						x,
								this._graph_bottom
										+ (this.klass.LABEL_MARGIN * 2.0), // Coordinates
																			// of
																			// text
			...
			_draw_label : function(y_offset, index) {
				if (this.labels[index] && !this._labels_seen[index]) {
					...
					this._d.pointsize = this
							._scale_fontsize(this.marker_font_size);
					this._d.gravity = 'east';
					this._d.annotate_scaled(1, 1, this._graph_left
							- this.klass.LABEL_MARGIN * 2.0, y_offset,
							this.labels[index], this._scale);
					this._labels_seen[index] = true;
				}
			}
...
Bluff.Net = new JS.Class(
		Bluff.Base,
		{
			...
			draw : function() {
				...
				this._center_y = this._graph_top + (this._graph_height / 2.0)
						- 10; // Move graph up a bit

				this._x_increment = this._graph_width
						/ (this._column_count - 1);
				var circle_radius = this.dot_radius
						|| this
								._clip_value_if_greater_than(
										this._columns
												/ (this._norm_data[0][this.klass.DATA_VALUES_INDEX].length * 2.5),
										7.0);
				...
				this._d.stroke_width = this.line_width
						|| this
								._clip_value_if_greater_than(
										this._columns
												/ (this._norm_data[0][this.klass.DATA_VALUES_INDEX].length * 4),
										3.0);
											...
											.each(
													...
													function(data_point, index) {
														...
														var rad_pos = index
																* Math.PI
																* 2
																/ this._column_count, point_distance = data_point
																* this._radius, start_x = this._center_x
																+ Math
																		.sin(rad_pos)
																* point_distance, start_y = this._center_y
																- Math
																		.cos(rad_pos)
																* point_distance,

														next_index = (index + 1 < data_row[this.klass.DATA_VALUES_INDEX].length) ? index + 1
																: 0,

														next_rad_pos = next_index
																* Math.PI
																* 2
																/ this._column_count, next_point_distance = data_row[this.klass.DATA_VALUES_INDEX][next_index]
																* this._radius, end_x = this._center_x
																+ Math
																		.sin(next_rad_pos)
																* next_point_distance, end_y = this._center_y
																- Math
																		.cos(next_rad_pos)
																* next_point_distance;
			...
			_draw_line_markers : function() {
				...
				// because this funtion is called before the @radius, @center_x
				// and @center_y are set
				this._radius = this._graph_height / 2.0;
				this._center_x = this._graph_left + (this._graph_width / 2.0);
				this._center_y = this._graph_top + (this._graph_height / 2.0)
						- 10; // Move graph up a bit
				...
				for ( var index = 0, n = this._column_count; index < n; index++) {
					...
					this._d.line(this._center_x, this._center_y, this._center_x
							+ Math.sin(rad_pos) * this._radius, this._center_y
							- Math.cos(rad_pos) * this._radius);

					marker_label = this.labels[index] ? this.labels[index]
							: '000';

					this._draw_label(this._center_x, this._center_y, rad_pos
							* 360 / (2 * Math.PI), this._radius, marker_label);
				}
			},

			_draw_label : function(center_x, center_y, angle, radius, amount) {
				var r_offset = 1.1, x_offset = center_x, // + 15 // The label
															// points need to be
															// tweaked slightly
				y_offset = center_y, // + 0 // This one doesn't though
				rad_pos = angle * Math.PI / 180, x = x_offset
						+ (radius * r_offset * Math.sin(rad_pos)), y = y_offset
						- (radius * r_offset * Math.cos(rad_pos));
...
Bluff.Pie = new JS.Class(
		Bluff.Base,
		{
			...
			// Can be used to make the pie start cutting slices at the top
			// (-90.0)
			// or at another angle. Default is 0.0, which starts at 3 o'clock.
			zero_degreee : null,

			// Do not show labels for slices that are less than this percent.
			// Use 0 to always show all labels.
			...
			draw : function() {
				...
				var diameter = this._graph_height, radius = (Math.min(
						this._graph_width, this._graph_height) / 2.0) * 0.8, top_x = this._graph_left
						+ (this._graph_width - diameter) / 2.0, center_x = this._graph_left
						+ (this._graph_width / 2.0), center_y = this._graph_top
						+ (this._graph_height / 2.0) - 10, // Move graph up a
															// bit
				total_sum = this._sums_for_pie(), prev_degrees = this.zero_degree, index = this.klass.DATA_VALUES_INDEX;
						...
						.each(
								...
								function(data_row, i) {
									if (data_row[this.klass.DATA_VALUES_INDEX][0] > 0) {
										...
										// Gruff uses ellipse() here, but canvas
										// doesn't seem to support it.
										// circle() is fine for our purposes
										// here.
										this._d
												.circle(
														center_x,
										...
										var half_angle = prev_degrees
												+ ((prev_degrees + current_degrees) - prev_degrees)
												/ 2, label_val = Math
												.round((data_row[this.klass.DATA_VALUES_INDEX][0] / total_sum) * 100.0), label_string;

										if (label_val >= this.hide_labels_less_than) {
											label_string = this
													._label(data_row[this.klass.DATA_VALUES_INDEX][0]);
											this
													._draw_label(
															center_x,
															center_y,
															half_angle,
															radius
																	+ (radius * this.klass.TEXT_OFFSET_PERCENTAGE),
															label_string);
										}
			...
			_draw_label : function(center_x, center_y, angle, radius, amount) {
				...
				var r_offset = 20.0, // The distance out from the center of
										// the pie to get point
				x_offset = center_x, // + 15.0 # The label points need to be
										// tweaked slightly
				y_offset = center_y, // This one doesn't though
				radius_offset = radius + r_offset, ellipse_factor = radius_offset * 0.15, x = x_offset
						+ ((radius_offset + ellipse_factor) * Math.cos(angle
								* Math.PI / 180)), y = y_offset
						+ (radius_offset * Math.sin(angle * Math.PI / 180));
...
Bluff.SideBar = new JS.Class(
		Bluff.Base,
		{
			...
			draw : function() {
				...
				this._bar_width = this._bars_width * this.bar_spacing
						/ this._norm_data.length;
				this._d.stroke_opacity = 0.0;
				var height = Bluff.array_new(this._column_count, 0), length = Bluff
						.array_new(this._column_count, this._graph_left), padding = (this._bars_width * (1 - this.bar_spacing)) / 2;
									...
									Bluff
											.each(
													...
													function(data_point,
														...
														// chart we wish to
														// stack.
														var temp1 = this._graph_left
																+ (this._graph_width
																		- data_point
																		* this._graph_width - height[point_index]), temp2 = this._graph_left
																+ this._graph_width
																- height[point_index], difference = temp2
																- temp1,

														left_x = length[point_index] - 1, left_y = this._graph_top
																+ (this._bars_width * point_index)
																+ (this._bar_width * row_index)
																+ padding, right_x = left_x
																+ difference, right_y = left_y
																+ this._bar_width;
														...
														var label_center = this._graph_top
																+ (this._bars_width
																		* point_index + this._bars_width / 2);
														this._draw_label(
																label_center,
																point_index);
													}, this)
			...
			_draw_line_markers : function() {
				...
				// 0.1, 0.5, etc.
				var increment = this._significant(this.maximum_value
						/ number_of_lines), line_diff, x, diff, marker_label;
				for ( var index = 0; index <= number_of_lines; index++) {

					line_diff = (this._graph_right - this._graph_left)
							/ number_of_lines;
...
Bluff.Spider = new JS.Class(
		Bluff.Base,
		{
			...
			draw : function() {
				...
				var diameter = this._graph_height, radius = this._graph_height / 2.0, top_x = this._graph_left
						+ (this._graph_width - diameter) / 2.0, center_x = this._graph_left
						+ (this._graph_width / 2.0), center_y = this._graph_top
						+ (this._graph_height / 2.0) - 25; // Move graph up a
															// bit

				this._unit_length = radius / this._max_value;

				var total_sum = this._sums_for_spider(), prev_degrees = 0.0, additive_angle = (2 * Math.PI)
						/ this._data.length,
				...
				// Draw axes
				if (!this.hide_axes)
					this._draw_axes(center_x, center_y, radius, additive_angle);
			...
			_draw_label : function(center_x, center_y, angle, radius, amount) {
				var r_offset = 50, // The distance out from the center of the
									// pie to get point
				x_offset = center_x, // The label points need to be tweaked
										// slightly
				y_offset = center_y + 0, // This one doesn't though
				x = x_offset + ((radius + r_offset) * Math.cos(angle)), y = y_offset
						+ ((radius + r_offset) * Math.sin(angle));
			...
			_draw_axes : function(center_x, center_y, radius, additive_angle,
					line_color) {
				if (this.hide_axes)
					return;
				...
				Bluff.each(this._data, function(data_row) {
					...
					this._d.line(center_x, center_y, center_x + x_offset,
							center_y + y_offset);

					if (!this.hide_text)
						this._draw_label(center_x, center_y, current_angle,
								radius, data_row[this.klass.DATA_LABEL_INDEX]);
						...
						.each(
								...
								function(data_row) {
									points
											.push(center_x
													+ this
															._normalize_points(data_row[this.klass.DATA_VALUES_INDEX][0])
...
Bluff.Base.StackedMixin = new JS.Module( {
	...
	_get_maximum_by_stack : function() {
		...
		Bluff.each(this._data, function(data_set) {
			Bluff.each(data_set[this.klass.DATA_VALUES_INDEX], function(
					data_point, i) {
				if (!max_hash[i])
					max_hash[i] = 0.0;
		...
		for ( var key in max_hash) {
			if (max_hash[key] > this.maximum_value)
				this.maximum_value = max_hash[key];
		}
...
Bluff.StackedArea = new JS.Class(Bluff.Base, {
	...
	draw : function() {
		...
		var iterator = this.last_series_goes_on_bottom ? 'reverse_each'
				: 'each';
		Bluff[iterator](this._norm_data, function(data_row) {
			...
			Bluff.each(data_row[this.klass.DATA_VALUES_INDEX],
					function(data_point, index) {
						...
						var new_y = this._graph_top
								+ (this._graph_height - data_point
										* this._graph_height - height[index]);
...
Bluff.StackedBar = new JS.Class(
		Bluff.Base,
		{
			...
			// Draws a bar graph, but multiple sets are stacked on top of each
			// other.
			draw : function() {
				this._get_maximum_by_stack();
				this.callSuper();
											...
											.each(
													...
													function(data_point,
														...
														var left_x = this._graph_left
																+ (this._bar_width * point_index)
																+ padding;
														var left_y = this._graph_top
																+ (this._graph_height
																		- data_point
																		* this._graph_height - height[point_index])
																+ 1;
														var right_x = left_x
																+ this._bar_width
																* this.bar_spacing;
														var right_y = this._graph_top
																+ this._graph_height
																- height[point_index]
																- 1;
...
Bluff.AccumulatorBar = new JS.Class(Bluff.StackedBar, {
	...
	draw : function() {
		...
		var accumulator_array = [], index = 0, increment_array = [];

		Bluff.each(this._data[0][this.klass.DATA_VALUES_INDEX],
				function(value) {
					var max = -Infinity;
					Bluff.each(increment_array, function(x) {
						max = Math.max(max, x);
					});
...
Bluff.SideStackedBar = new JS.Class(
		Bluff.SideBar,
		{
			include : Bluff.Base.StackedMixin,
			...
			draw : function() {
				...
				var height = Bluff.array_new(this._column_count, 0), length = Bluff
						.array_new(this._column_count, this._graph_left), padding = (this._bar_width * (1 - this.bar_spacing)) / 2;
											...
											.each(
													...
													function(data_point,
														...
														var temp1 = this._graph_left
																+ (this._graph_width
																		- data_point
																		* this._graph_width - height[point_index])
																+ 1;
														var temp2 = this._graph_left
																+ this._graph_width
																- height[point_index]
																- 1;
														var difference = temp2
																- temp1;

														var left_x = length[point_index], // + 1
														left_y = this._graph_top
																+ (this._bar_width * point_index)
																+ padding, right_x = left_x
																+ difference, right_y = left_y
																+ this._bar_width
...
Bluff.Mini.Legend = new JS.Module(
		{
			...
			// The canvas needs to be bigger so we can put the legend beneath
			// it.
			_expand_canvas_for_vertical_legend : function() {
				...
				var legend_height = this._scale_fontsize(this._data.length
						* this._calculate_line_height() + this.top_margin
						+ this.bottom_margin);
				...
				case 'right':
					...
					this._columns += this._calculate_legend_width()
							+ this.left_margin;
					break;
			...
			_calculate_legend_width : function() {
				...
				Bluff.each(this._legend_labels, function(label) {
					width = Math.max(this._calculate_width(
							this.legend_font_size, label), width);
				}, this);
			...
			_draw_vertical_legend : function() {
				...
				var legend_square_width = 40.0, // small square with color of
												// this item
				legend_square_margin = 10.0, legend_left_margin = 100.0, legend_top_margin = 40.0;
				...
				switch (this.legend_position) {
				case 'right':
					current_x_offset = this._original_columns
							+ this.left_margin;
				...
				default:
					current_x_offset = legend_left_margin,
							current_y_offset = this._original_rows
									+ legend_top_margin;
					break;
				}

				this._debug(function() {
					this._d.line(0.0, current_y_offset, this._raw_columns,
							current_y_offset);
				});
								...
								function(legend_label, index) {
									...
									this._d
											.annotate_scaled(
													...
													current_y_offset,
													this
															._truncate_legend_label(legend_label),
													this._scale);
			...
			_truncate_legend_label : function(label) {
				...
				while (this._calculate_width(this
						._scale_fontsize(this.legend_font_size),
						truncated_label) > (this._columns
						- this.legend_left_margin - this.right_margin)
						&& (truncated_label.length > 1))
					truncated_label = truncated_label.substr(0,
							truncated_label.length - 1);
				return truncated_label
						+ (truncated_label.length < String(label).length ? "..."
								: '');
			}
		});
...
Bluff.Renderer = new JS.Class(
		{
			...
			caps_height : function(font_size) {
				var X = this._sized_text(font_size, 'X'), height = this
						._element_size(X).height;
				this._remove_node(X);
				return height;
			},
			...
			render_gradiated_background : function(width, height, top_color,
					bottom_color) {
			...
			annotate_scaled : function(width, height, x, y, text, scale) {
				...
				var scaled_height = (height * scale) >= 1 ? (height * scale)
						: 1;
				var text = this._sized_text(this.pointsize, text);
				text.style.color = this.fill;
				text.style.fontWeight = this.font_weight;
				text.style.textAlign = 'center';
				text.style.left = (this._sx * x + this._left_adjustment(text,
						scaled_width))
						+ 'px';
				text.style.top = (this._sy * y + this._top_adjustment(text,
						scaled_height))
						+ 'px';
			},
			...
			tooltip : function(left, top, width, height, name, color, data) {
				...
				var wrapper = this._canvas.parentNode, target = document
						.createElement('div');
			...
			circle : function(origin_x, origin_y, perim_x, perim_y, arc_start,
					arc_end) {
				var radius = Math.sqrt(Math.pow(perim_x - origin_x, 2)
						+ Math.pow(perim_y - origin_y, 2));
				...
				if (arc_start !== undefined && arc_end !== undefined
					...
					this._ctx.moveTo(this._sx
							* (origin_x + radius * Math.cos(beta)), this._sy
							* (origin_y + radius * Math.sin(beta)));
					this._ctx.lineTo(this._sx * origin_x, this._sy * origin_y);
					this._ctx.lineTo(this._sx
							* (origin_x + radius * Math.cos(alpha)), this._sy
							* (origin_y + radius * Math.sin(alpha)));
				}
				this._ctx.arc(this._sx * origin_x, this._sy * origin_y,
						this._sx * radius, alpha, beta, false); // draw it
																// clockwise
				this._ctx.fill();
			},
			...
			rectangle : function(ax, ay, bx, by) {
				...
				try {
					this._ctx.fillStyle = this.fill;
					this._ctx.fillRect(this._sx * ax, this._sy * ay, this._sx
							* (bx - ax), this._sy * (by - ay));
				} catch (e) {
				}
				try {
					this._ctx.strokeStyle = this.stroke;
					if (this.stroke !== 'transparent')
						this._ctx.strokeRect(this._sx * ax, this._sy * ay,
								this._sx * (bx - ax), this._sy * (by - ay));
				} catch (e) {
				}
			},
			...
			_text_container : function() {
				...
				if (wrapper.className === this.klass.WRAPPER_CLASS)
					return wrapper;
			...
			_sized_text : function(size, content) {
				...
				text.style.fontSize = (typeof size === 'number') ? size + 'px'
						: size;
				return text;
			},
...
Bluff.Event = {
	...
	observe : function(element, eventName, callback, scope) {
		...
		if (Bluff.index(handlers, callback) !== -1)
			return;
	...
	stopObserving : function(element) {
		...
		Bluff.each(handlers, function(entry) {
			if (entry._node.removeEventListener)
				entry._node.removeEventListener(entry._name, entry._responder,
						false);
			else
	...
	_handlersFor : function(element, eventName) {
		...
		Bluff.each(this._cache, function(entry) {
			if (element && entry._node !== element)
				return;
			if (eventName && entry._name !== eventName)
				return;
			results.push(entry);
		});
		return results;
...
Bluff.Tooltip = new JS.Singleton( {
	...
	show : function(name, color, data) {
		...
		this._tip.innerHTML = '<span class="color" style="background: ' + color
				+ ';">&nbsp;</span> ' + '<span class="label">' + name
				+ '</span> ' + '<span class="data">' + data + '</span>';
...
Bluff.TableReader = new JS.Class(
		{
			...
			initialize : function(table, transpose) {
				this._table = (typeof table === 'string') ? document
						.getElementById(table) : table;
