...
var MultiBox = new Class({
  ...
  setContentType: function(link){
    ...
    switch(str){
      ...
      default:
      	...
      	if (link.href.match(/youtube\.com\/watch/i))
      	{
  ...
  getContent: function(index){
    ...
    this.contentToLoad = {
      title: this.options.titleOfContent(this.content[index]),
  ...
  createEmbedObject: function(){
    ...
    if(this.type == 'flashVideo'){
      ...
      // Patch for external video rtmp/rtmpt
      if (url.indexOf('rtmp:') === 0 || url.indexOf('rtmpt:') === 0) {
    	  var file = url.split('/')[url.split('/').length - 1];
    	  url = file + "&streamer=" + url.substring(0, url.length - file.length);
      }
...
/*
    ...
    if(this.type == 'youtubeVideo'){
      ...
      this.str = '<embed id="bannerswf" width="' + this.contentObj.width + '" height="' + (this.contentObj.height + this.contentObj.xH) + '" ';
      this.str+= ' flashvars="file=' + url + '&type=youtube"';
      this.str+= ' wmode="opaque" allowscriptaccess="always" allowfullscreen="true" quality="high" bgcolor="#ffffff" type="application/x-shockwave-flash" name="swf"';
      this.str+= ' src="' + this.options.path + 'player.swf"/>';
