...
public class SearchFilterStatus extends SearchFilterOptionProvider
{
    ...
    public List<Criterion> getCriterionList(String parameter, HttpServletRequest request)
    {
        ...
        {
            if (!values[0].equals("ALL") && "PUB".equals(values[0]))
            {
                criterionList.add(Restrictions.eq(META_DATA_MGNL_ACTIVATED, "true"));

            }
            if (!values[0].equals("ALL") && "NOTPUB".equals(values[0]))
            {
                criterionList.add(Restrictions.eq(META_DATA_MGNL_ACTIVATED, false));
            }
        }
