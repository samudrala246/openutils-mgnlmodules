 ...
 * @version $Id$
 */
public class DateGridColumnType extends AbstractGridColumnType
{
    ...
    protected void addColumnData(Map<String, String> column, String propertyName, int colIndex, Map colMap,
        Messages msgs)
    {
        column.put("editor", "new Ed(new fm.DateField({format: 'Y-m-d'}))");
        column.put("renderer", "function(v){ return (v && Ext.isDate(v)) ? v.format('Y-m-d') : v; }");
    }
}
